...
```
tools/                    → SOURCE code (packaged, committed to git)
  ├── progressive-reader/ → Tool source + install.sh
  └── progressive-reader-wrapper/ → Wrapper scripts + tool.json

.claude/                  → INSTALLED runtime files (generated, NOT committed)
  ├── tools/             → Installed copies from tools/
  ├── hooks/             → Installed copies from hooks/
...
- ✅ CREATE in root directories: `tools/`, `hooks/`, `scripts/`
- ❌ DO NOT create in `.claude/` - this is for installed files only
...
tools/progressive-reader-wrapper/
  ├── progressive-reader.sh
  └── tool.json

# WRONG - Creating in installed location
.claude/tools/progressive-reader/  ❌ DON'T DO THIS!
```
